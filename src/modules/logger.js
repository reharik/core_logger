...
var Logger = function(options){
    ...
    function addSqsLogstashSink(options){
        validate(internals.schema.sqslogstash, options);
        internals.options.sqslogstash = _.assign(internals.options.sqslogstash, options);
        internals.context.logger.add(Winston.transports.SqsLogstash, internals.options.sqslogstash);
        return this;
    }

    function addTestSink(){
        internals.context.logger = new TestLogger();
        return this;
    }
    ...
    function trace(context){
        if(internals.context.logger.silly){
            internals.context.logger.silly(mapContext(context, "trace"));
        } else {
            internals.context.logger.trace(mapContext(context, "trace"));
        }
        return this;
    }
    ...
    return {
        ...
        addSqsLogstashSink : addSqsLogstashSink,
        addTestSink : addTestSink,
        error : error,
        warn : warn,
        info : info,
        debug : debug,
